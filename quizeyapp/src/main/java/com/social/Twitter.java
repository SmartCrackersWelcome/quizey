...
public class Twitter {
	...
	private String twitter_consumer_key = "Yt1LQxF2mMS8pEI896VSaZovG";
	private String twitter_consumer_secret = "0bB16Z7lZisOpMew21bw9wZn1cnI3mrdgoa4JTWHPLZndkpeto";
	 ...
	 * call to api.twitter.com/oauth/request_token INPUT: nothing OUTPUT: if
	 * successful, twitter API will return oauth_token, oauth_token_secret and
	...
	public JSONObject startTwitterAuthentication() {
		...
		try {
			oauth_signature = computeSignature(signature_base_string, twitter_consumer_secret + "&"); // note
																										// the
																										// &
																										// at
																										// the
		...
		System.out.println("authorization_header_string=" + authorization_header_string); // print
																							// out
		...
		try { // jsonexception block
			try {
				...
				} else // returned 200 (good)
				{
					...
					System.out.println("endpoint startTwitterAuthentication responsebody=" + responseBody); // print
																											// out
																											// the
																											// response
																					...
																					// oauth_callback_confirmed?
					{
								...
								// return them.
						{
							...
							if (oauth_token.equals("") || oauth_token_secret.equals("")) // if
																							// either
																							// key
																							// is
																							// empty,
				...
				jsonresponse.put("response_status", "error");
				jsonresponse.put("message",
						"startTwitterAuthentication NoSuchAlgorithmException message=" + nsae.getMessage());
			} catch (KeyManagementException kme) {
				System.out.println(kme.getMessage());
				jsonresponse.put("response_status", "error");
				jsonresponse.put("message",
						"startTwitterAuthentication KeyManagementException message=" + kme.getMessage());
			} finally {
				conn.close();
			}
	 ...
	 * authorization page. They grant access and either a) are shown a pin
	 * number b) sent to the callback url with information In either case, turn
	 * the authorization code into a twitter access token for that user.
	 *
	 * My example here is uses a pin and oauth_token (from the previous request
	 * token call) INPUT: pin, generic request token OUTPUT: if successful,
	 * twitter API will return access_token, access_token_secret, screen_name
	 * and user_id
	 * 
	 */
	public JSONObject getTwitterAccessTokenFromAuthorizationCode(String verifier_or_pin, String oauth_token) {
		...
		try {
			try {
				...
				// the request
				request2.setEntity(new StringEntity("oauth_verifier=" + encode(verifier_or_pin),
						"application/x-www-form-urlencoded", "UTF-8"));
				request2.setParams(params);
				request2.addHeader("Authorization", authorization_header_string);
				httpexecutor.preProcess(request2, httpproc, context);
				HttpResponse response2 = httpexecutor.execute(request2, conn, context);
				System.out.println("getTwitterAccessTokenFromAuthorizationCode response.getStatusLine()="
																				...
																				// bad
				{
					...
					System.out.println("oauth_token=" + access_token);
					System.out.println("oauth_token_secret=" + access_token_secret);
				...
				jsonresponse.put("response_status", "error");
				jsonresponse.put("message",
						"getTwitterAccessTokenFromAuthorizationCode HttpException message=" + he.getMessage());
			} catch (NoSuchAlgorithmException nsae) {
				System.out.println(nsae.getMessage());
				jsonresponse.put("response_status", "error");
				jsonresponse.put("message",
				...
				System.out.println(kme.getMessage());
				jsonresponse.put("response_status", "error");
				jsonresponse.put("message", "getTwitterAccessTokenFromAuthorizationCode KeyManagementException message="
						+ kme.getMessage());
			} finally {
				conn.close();
			}
	...
	/*
	 * This is the search example, using a GET call INPUT: the search query (q),
	 * the user's access_token and the user's access_token_secret OUTPUT: if
	 * successful, twitter API will return a json object of tweets
	 * 
	 */
	public JSONObject searchTweets(String q, String access_token, String access_token_secret) {
		...
		// and "q".
		String parameter_string = "lang=en&oauth_consumer_key=" + twitter_consumer_key + "&oauth_nonce=" + oauth_nonce
				+ "&oauth_signature_method=" + oauth_signature_method + "&oauth_timestamp=" + oauth_timestamp
				+ "&oauth_token=" + encode(oauth_token) + "&oauth_version=1.0&q=" + encode(q) + "&result_type=mixed";
		System.out.println("parameter_string=" + parameter_string);
		...
		try {
			try {
				...
				BasicHttpEntityEnclosingRequest request2 = new BasicHttpEntityEnclosingRequest("GET",
						twitter_endpoint_path + "?lang=en&result_type=mixed&q=" + encode(q));
				request2.setParams(params);
				...
				if (response2.getStatusLine().toString().indexOf("500") != -1) {
					...
					if (jo.has("errors")) {
						...
						if (message_from_twitter.equals("Invalid or expired token")
							...
							jsonresponse.put("message",
									jo.getJSONArray("errors").getJSONObject(0).getString("message"));
					} else {
	...
	/*
	 * This is the update status example Urls (even short ones) will be wrapped
	 * and be counted as a certain length defined by Twitter configuration (~22
	 * chars as of this writing). There is a 140 char max, including this
	 * limitation INPUT:the user's access_token and the user's
	 * access_token_secret and the text of the status update OUTPUT: if
	 * successful, the tweet gets posted.
	 * 
	 */
	public JSONObject updateStatus(String access_token, String access_token_secret, String text) {
		...
		try {
			try {
				...
				request2.setEntity(
						new StringEntity("status=" + encode(text), "application/x-www-form-urlencoded", "UTF-8"));
				request2.setParams(params);
				request2.addHeader("Authorization", authorization_header_string);
				httpexecutor.preProcess(request2, httpproc, context);
				HttpResponse response2 = httpexecutor.execute(request2, conn, context);
	 ...
	 * returns a valid JSONObject, regardless if it spoke to twitter and did the
	 * update succesfully, it returns just the object. if any error occurs,
	 * whether from twitter or failing to talk to twitter entirely, an
	 * response_status=error is sent back
	 * 
	 */
	public JSONObject updateStatusWithMedia(String access_token, String access_token_secret, String text, File file)
		...
		String authorization_header_string = "OAuth oauth_consumer_key=\"" + twitter_consumer_key
				...
				+ "\",oauth_token=\"" + encode(oauth_token) + "\"";
		System.out
				.println("Twitter.updateStatusWithMedia(): authorization_header_string=" + authorization_header_string);
		...
		try {
			try {
				...
				// System.out.println("Twitter.updateStatusWithMedia(): params
				// all set, creating socket.");
				// Socket socket = new Socket(host.getHostName(),
				// host.getPort());
				// conn.bind(socket, params);
	...
	public JSONObject getTweet(String access_token, String access_token_secret, String id) {
		...
		// this time, I add 3 extra params to the request, "lang", "result_type"
		// and "q".
		String parameter_string = "id=" + id + "&oauth_consumer_key=" + twitter_consumer_key + "&oauth_nonce="
	...
	public static void main(String[] args) {
		...
		Gson g = new Gson();
		// g.toJson(twitter.startTwitterAuthentication());
		g.toJson(twitter.getTwitterAccessTokenFromAuthorizationCode("Gj0NmEmEjoQInRMymcF9MJ1AnVEhsOiJ",
				"_L6BUgAAAAAA0IjiAAABW92eym8"));
	}
}
